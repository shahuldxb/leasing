...
export default function CPIEscalation() {
  ...
  const pending: any[] = [];
  const refetch = () => {};
  const applyMut = { mutate: (_: any) => { toast.success('CPI escalation applied'); setApplyDialog(null); }, isPending: false };
  const displayRows = aiRows.length > 0 ? aiRows : (pending as any[]);
        ...
        <div className="flex flex-col h-full w-full bg-background">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-card shrink-0">
            ...
            <div className="flex gap-2">
              ...
              <Button disabled={applyMut.isPending} onClick={() => applyMut.mutate({ review_id: applyDialog.review_id, new_rent: Number(newRent) })}>
          ...
          <div className="flex-1 overflow-y-auto px-8 py-6">
            <div className="max-w-xl mx-auto space-y-5">
              ...
              <div><Label className="text-xs text-muted-foreground">Confirmed New Rent (AED)</Label><Input type="number" step="0.01" className="mt-1" value={newRent} onChange={e => setNewRent(e.target.value)} placeholder={String(applyDialog.proposed_rent ?? "")} /></div>
            </div>
          </div>
        </div>
      ) : (
        <div className="p-6 space-y-6">
          <ScreenHeader screenId="VFLCPIESC0001P001" title="CPI Escalation" subtitle="Consumer Price Index rent escalations pending review and application"
            screenType="cpi_escalation" onAIData={(rows) => setAiRows(rows)} />
          ...
          <Card><CardContent className="p-0"><Table>
            <TableHeader><TableRow><TableHead>Contract</TableHead><TableHead>Property</TableHead><TableHead className="text-right">Current Rent</TableHead><TableHead className="text-right">CPI Rate</TableHead><TableHead className="text-right">Proposed Rent</TableHead><TableHead>Effective Date</TableHead><TableHead>Status</TableHead><TableHead></TableHead></TableRow></TableHeader>
            <TableBody>
              {displayRows.map((r: any, i: number) => (
                <TableRow key={i}>
                  <TableCell className="font-mono text-xs">{r.contract_ref}</TableCell>
                  <TableCell className="text-sm">{r.property_name ?? "—"}</TableCell>
                  ...
                  <TableCell className="text-sm">{r.effective_date?.slice(0,10)}</TableCell>
                  <TableCell>{statusBadge(r.status)}</TableCell>
                  <TableCell>{r.status === "PENDING" && <Button size="sm" variant="outline" onClick={() => { setNewRent(String(r.proposed_rent ?? "")); setApplyDialog(r); }}><TrendingUp className="w-3 h-3 mr-1" />Apply</Button>}</TableCell>
                </TableRow>
              ))}
              {displayRows.length === 0 && <TableRow><TableCell colSpan={8} className="text-center py-6 text-muted-foreground">No CPI escalations pending</TableCell></TableRow>}
            </TableBody>
