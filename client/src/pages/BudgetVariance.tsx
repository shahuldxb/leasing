      ...
      {bvOpen ? (
        <div className="flex flex-col h-full w-full bg-background">
          ...
          <div className="flex-1 overflow-y-auto px-8 py-6">
            <div className="max-w-xl mx-auto grid grid-cols-2 gap-5">
              ...
              <div><Label className="text-xs text-muted-foreground">Budgeted Amount (AED)</Label><Input type="number" step="0.01" className="mt-1" value={form.budgeted_amount} onChange={e => setForm(f => ({ ...f, budgeted_amount: Number(e.target.value) }))} /></div>
              <div><Label className="text-xs text-muted-foreground">Actual Amount (AED)</Label><Input type="number" step="0.01" className="mt-1" value={form.actual_amount} onChange={e => setForm(f => ({ ...f, actual_amount: Number(e.target.value) }))} /></div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: "Total Budget", value: `AED ${totalBudget.toLocaleString()}`, color: "text-blue-600" },
              { label: "Total Actual", value: `AED ${totalActual.toLocaleString()}`, color: "text-foreground" },
              { label: "Variance", value: `AED ${Math.abs(totalVariance).toLocaleString()}`, color: totalVariance > 0 ? "text-red-600" : "text-emerald-600", icon: totalVariance > 0 ? <TrendingUp className="w-4 h-4 text-red-500" /> : <TrendingDown className="w-4 h-4 text-emerald-500" /> },
            ].map(k => (
