...
async function run() {
  ...
  const migSql = fs.readFileSync(path.join(__dirname, 'server/migrations/fix_fx_reval_sp_v4.sql'), 'utf8');
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    catch(e) { console.error(`Batch ${i+1} FAILED: ${e.message.slice(0,150)}`); }
  }
